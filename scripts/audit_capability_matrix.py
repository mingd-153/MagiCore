...
def main() -> int:
    ...
    # the parent directory before writing (the matrix must generate on
    # every runner, not just machines with a local docs/specs).
    # Checkout CI không có cây docs/specs (bị gitignored) — tạo thư mục
    # cha trước khi ghi (matrix phải sinh được trên mọi runner, không
    # chỉ máy có docs/specs local).
