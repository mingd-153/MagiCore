...
fn find_mgc_binary() -> String {
    ...
    if debug.exists() {
        ...
        panic!(
            "mgc binary not found at {:?} or {:?}. Run: cargo build -p mgc",
            debug, release
        );
    }
}
...
fn test_web_lifecycle_create_only() {
    ...
    assert!(output.status.success(), "mgc create failed:\n{}", combined);

    assert!(project_path.exists(), "Project directory not created");

    // VERIFY: package.json exists
    let package_json = project_path.join("package.json");
    assert!(package_json.exists(), "package.json not created");

    // VERIFY: .mgc.core marker
    let mgc_core = project_path.join(".mgc.core");
    assert!(mgc_core.exists(), ".mgc.core marker not created");
...
fn test_app_lifecycle_create_only() {
    ...
    assert!(pubspec.exists(), "pubspec.yaml not created");
