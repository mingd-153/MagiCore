...
fn get(key: &str) -> Result<()> {
    ...
    if let Some(value) = env_value(key) {
        mgc_ui::info(&format!("[env] {key} = {value}"));
        println!("{value}");
        return Ok(());
    }
    // 2. mgc.toml project
    ...
    if let Some(value) = file_value(&project_npmrc, key) {
        mgc_ui::info(&format!("[.npmrc local] {key} = {value}"));
        println!("{value}");
        return Ok(());
    }
    // 4. .npmrc user (~/)
    if let Some(home) = dirs::home_dir() {
        ...
        if let Some(value) = file_value(&user_npmrc, key) {
            mgc_ui::info(&format!("[.npmrc user] {key} = {value}"));
            println!("{value}");
            return Ok(());
        }
    }
...
fn set_npmrc(path: &Path, key: &str, value: &str) -> Result<()> {
    ...
    mgc_ui::success(&format!("set {key} = {value}  →  {}", path.display()));
    Ok(())
}
...
fn set_toml(key: &str, value: &str) -> Result<()> {
    ...
    mgc_ui::success(&format!("set {key} = {value}  →  {}", toml_path.display()));
    Ok(())
}
