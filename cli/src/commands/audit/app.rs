//! App core audit — dispatch to the adapter's real scanner via the shared
//! pipeline (detect → adapter → scanner → typed report → exit contract).
//! Audit core App — dispatch sang scanner thật của adapter qua pipeline
...
pub(crate) async fn audit(ctx: &ProjectContext, fmt: OutputFormat) -> anyhow::Result<()> {
    ...
    finish_and_print("app", &report, StrictMode::from_env(), fmt).await
}
