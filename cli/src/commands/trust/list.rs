//! mg trust list — List keys in keyring
//! mg trust list — Liệt kê keys trong keyring

use mg_crypto::keyring::Keyring;

/// Execute `mg trust list` — Thực thi `mg trust list`
pub fn execute() -> anyhow::Result<()> {
    ...
    if !keyring_path.exists() {
        ...
        println!("  Run 'mg trust init' to create one");
        return Ok(());
    }
