...
//! Chuyển đổi lockfile legacy (package-lock.json, pnpm-lock.yaml, yarn.lock, bun.lock) sang định dạng mgc.lock
//!
...
/// Chuyển đổi lockfile cũ thành mgc.lock chuẩn xác và an toàn.
pub async fn run(_project_dir: Option<std::path::PathBuf>) -> Result<()> {
    anyhow::bail!(
        "mgc import is temporarily disabled during lockfile v2 migration.\n\
