//! Integration tests for `mg trust` workflow
//! Test tích hợp cho workflow `mg trust`

use mg_crypto::keyring::{KeyPair, Keyring};
use mg_lockfile::{load_lockfile, sign_and_write_lockfile, verify_lockfile, VerificationStatus};
use std::fs;
...
"#;
    ...
    // Step 2: Initialize keyring (mg trust init)
    ...
    // Step 4: Sign lockfile (mg trust sign)
    sign_and_write_lockfile(&mut lockfile, &lockfile_path, &key_pair).unwrap();
    
    // Step 5: Verify lockfile (mg trust verify)
