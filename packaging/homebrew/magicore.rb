class Magicore < Formula
  ...
  # DO-NOT-PUBLISH placeholders — the v1.1.0-rc.9 archives do not exist
  # yet, so NO real hash can be computed. Publishing this manifest as-is
  # would guarantee install failure (hash mismatch). Rerun
  # scripts/update-release-hashes.sh after the v1.1.0-rc.9 release assets
  ...
  # CẤM-PUBLISH — archive v1.1.0-rc.9 chưa tồn tại nên KHÔNG THỂ tính hash
  ...
  # release v1.1.0-rc.9 có thật, rồi xác nhận `python3
  ...
  on_macos do
    if Hardware::CPU.arm?
      ...
      sha256 "PENDING_RC9_SHA256_DO_NOT_PUBLISH"
    else
  ...
  on_linux do
    if Hardware::CPU.arm? && Hardware::CPU.is_64_bit?
      ...
      sha256 "PENDING_RC9_UNAVAILABLE_NO_LINUX_ARM64_ARCHIVE"
    else
