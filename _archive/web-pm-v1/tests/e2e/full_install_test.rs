...
use mg_installer::installer::{InstallOptions, Installer};
...
"#;
    ...
    let opts = InstallOptions {
        ...
        virtual_store_path: root.join(".mg"),
        hoisted_node_modules: false,
        hoist_pattern: vec!["*".to_string()],
        offline: true,
        dry_run: false,
        project_root: root.clone(),
        sqlite_path: store_path.join("mg.db"),
        jsonl_log: false,
        linker_strategy: LinkerStrategy::Hoisted,
        gvs_root: PathBuf::from("/tmp/.mg").join("gvs").join("v1"),
    };
    ...
    let result = rt.block_on(async {
        ...
        let lockfile = mg_lockfile::text::read_text(&lf_path).expect("read lockfile");
