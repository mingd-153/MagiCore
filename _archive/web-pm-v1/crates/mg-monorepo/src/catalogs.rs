...
//!
//! Catalogs define version specs in a single place (`mg.yaml`) and let
...
impl CatalogResolver {
    ...
    pub fn from_config(config: &mg_core::MgpmConfig) -> Self {
...
mod test {
    ...
    fn default_config() -> mg_core::MgpmConfig {
        let mut cfg = mg_core::MgpmConfig::default();
        cfg.catalogs.insert(
            "default".to_string(),
            Catalog {
    ...
    fn test_rewrite_catalog_ref_error() {
        let config = mg_core::MgpmConfig::default();
        let resolver = CatalogResolver::from_config(&config);
