...
pub fn bench_lockfile_serialize_text(c: &mut Criterion) {
    ...
    for &n in [10, 100].iter() {
        ...
        group.bench_with_input(BenchmarkId::new("packages", n), &n, |b, _| {
            b.iter(|| {
                let path = dir.path().join("mg.lock");
                let _ = mg_lockfile::text::write_text(&lock, &path);
            });
        });
    }
    group.finish();
...
pub fn bench_lockfile_deserialize_text(c: &mut Criterion) {
    ...
    for &n in [10, 100].iter() {
        ...
        let dir = tempfile::tempdir().unwrap();
        let path = dir.path().join("mg.lock");
        mg_lockfile::text::write_text(&lock, &path).unwrap();

        group.bench_with_input(BenchmarkId::new("packages", n), &n, |b, _| {
            b.iter(|| {
                let _ = mg_lockfile::text::read_text(&path);
            });
        });
    }
    group.finish();
...
pub fn bench_lockfile_serialize_binary(c: &mut Criterion) {
    ...
    for &n in [10, 100].iter() {
        ...
        group.bench_with_input(BenchmarkId::new("packages", n), &n, |b, _| {
            b.iter(|| {
                let path = dir.path().join("mg.lockb");
                let _ = mg_lockfile::binary::write_binary(&lock, &path);
            });
        });
    }
    group.finish();
...
pub fn bench_lockfile_deserialize_binary(c: &mut Criterion) {
    ...
    for &n in [10, 100].iter() {
        ...
        let dir = tempfile::tempdir().unwrap();
        let path = dir.path().join("mg.lockb");
        mg_lockfile::binary::write_binary(&lock, &path).unwrap();

        group.bench_with_input(BenchmarkId::new("packages", n), &n, |b, _| {
            b.iter(|| {
                let _ = mg_lockfile::binary::read_binary(&path);
            });
        });
    }
    group.finish();
