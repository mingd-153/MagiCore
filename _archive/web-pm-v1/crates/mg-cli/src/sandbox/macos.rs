...
pub fn apply_seatbelt(project_dir: &Path) -> Result<SandboxGuard, String> {
    ...
    let profile = format!(
        ...
        (allow file-write* (subpath \"{home}/.mg\"))
    ...
    let profile_path = std::env::temp_dir().join(format!("mg-sandbox-{}.sb", std::process::id()));
    ...
    eprintln!("To apply: sandbox-exec -f {:?} mg install", profile_path);
