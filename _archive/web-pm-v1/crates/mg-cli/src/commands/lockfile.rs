...
fn cmd_lockfile_upgrade(to: String) -> Result<(), String> {
    let text_path = Path::new("mg.lock");
    let binary_path = Path::new("mg.lockb");

    let lockfile = if text_path.exists() {
        mg_lockfile::text::read_text(text_path)
            .map_err(|e| format!("failed to read text lockfile: {e}"))?
    } else if binary_path.exists() {
        mg_lockfile::binary::read_binary(binary_path)
            .map_err(|e| format!("failed to read binary lockfile: {e}"))?
    } else {
        return Err("no lockfile found (mg.lock or mg.lockb)".to_string());
    };

    match to.as_str() {
        "text" => {
            mg_lockfile::text::write_text(&lockfile, text_path)
                .map_err(|e| format!("failed to write text lockfile: {e}"))?;
            println!(
                "{} Written lockfile to text format (mg.lock)",
                "[OK]".green().bold()
            );
        }
        "binary" => {
            mg_lockfile::binary::write_binary(&lockfile, binary_path)
                .map_err(|e| format!("failed to write binary lockfile: {e}"))?;
            println!(
                "{} Written lockfile to binary format (mg.lockb)",
                "[OK]".green().bold()
            );
        }
        "both" => {
            ...
            mg_lockfile::binary::write_binary(&lockfile, binary_path)
                .map_err(|e| format!("failed to write binary lockfile: {e}"))?;
            println!(
                "{} Written lockfile to both formats (mg.lock, mg.lockb)",
                "[OK]".green().bold()
            );
        }
        _ => {
...
fn cmd_lockfile_validate() -> Result<(), String> {
    ...
    let lockfile = if text_path.exists() {
        ...
        eprintln!(
            "{} Reading lockfile from mg.lockb...",
            "[INFO]".cyan().bold()
        );
        mg_lockfile::binary::read_binary(binary_path)
            .map_err(|e| format!("failed to read binary lockfile: {e}"))?
    ...
    if lockfile.version != mg_lockfile::LOCKFILE_VERSION {
        issues.push(format!(
            "version mismatch: found {}, expected {}",
            lockfile.version,
            mg_lockfile::LOCKFILE_VERSION
        ));
    }
...
fn cmd_lockfile_migrate() -> Result<(), String> {
    ...
    if lockfile.version != mg_lockfile::lockfile::LOCKFILE_VERSION_V1 {
        return Err(format!(
