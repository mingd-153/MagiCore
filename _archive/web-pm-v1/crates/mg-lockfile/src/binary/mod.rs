//! Binary lockfile serialization (mg.lockb)
//!
...
use crate::{
    lockfile::Lockfile, lockfile::LOCKFILE_MAGIC, lockfile::LOCKFILE_VERSION,
    lockfile::LOCKFILE_VERSION_V1,
};
...
pub fn write_binary(lockfile: &Lockfile, path: &Path) -> Result<(), LockfileError> {
    let file = File::create(path).map_err(|e| LockfileError::Io(e.to_string()))?;
    let mut writer = BufWriter::new(file);

    writer
        .write_all(LOCKFILE_MAGIC)
    ...
    let encoded: Vec<u8> =
        bincode::serialize(lockfile).map_err(|e| LockfileError::Serialization(e.to_string()))?;
...
pub fn read_binary(path: &Path) -> Result<Lockfile, LockfileError> {
    let file = File::open(path).map_err(|e| LockfileError::Io(e.to_string()))?;
...
mod tests {
    ...
    fn test_binary_roundtrip() {
        ...
        let path = temp.path().join("mg.lockb");
