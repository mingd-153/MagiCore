//! Text lockfile serialization (mg.lock)
//!
...
use crate::{lockfile::LOCKFILE_VERSION_V1, LockfileError};
...
pub fn write_text(lockfile: &Lockfile, path: &Path) -> Result<(), LockfileError> {
    ...
    fs::write(path, content).map_err(|e: std::io::Error| LockfileError::Io(e.to_string()))?;
    Ok(())
}

pub fn read_text(path: &Path) -> Result<Lockfile, LockfileError> {
    let content =
        fs::read_to_string(path).map_err(|e: std::io::Error| LockfileError::Io(e.to_string()))?;
    ...
    let metadata = super::LockfileMetadata {
        ...
        created_at: metadata_table
            ...
            .and_then(|v| v.as_integer())
            .unwrap_or(0) as u64,
        updated_at: metadata_table
            .get("updated_at")
            .and_then(|v| v.as_integer())
            .unwrap_or(0) as u64,
        content_hash: metadata_table
            .get("content_hash")
            .and_then(|v| v.as_str())
            .unwrap_or("")
            .to_string(),
        registry: metadata_table
            .get("registry")
            .and_then(|v| v.as_str())
            .unwrap_or("npm")
            .to_string(),
    ...
    if is_v1 {
        lockfile.migrate_v1_to_v2().map_err(|e| {
            LockfileError::Corrupted(format!("v1 lockfile migration failed: {}", e))
        })?;
    }
...
mod tests {
    ...
    fn test_text_roundtrip() {
        ...
        let path = temp.path().join("mg.lock");
