...
impl PackageCache {
    ...
    pub fn default_cache_dir() -> Result<PathBuf> {
        ...
        Ok(home.join(".megagate").join("cache"))
    }
...
mod tests {
    ...
    fn test_cache_store_and_get() {
        ...
        fs::write(&pkg_file, b"fake package").unwrap();

        let integrity = cache.compute_integrity(&pkg_file).unwrap();

        // Store
        let cached = cache.store_package("react@18.2.0", &pkg_file, &integrity).unwrap();
        assert!(cached.exists());

        // Get (verify integrity)
        let retrieved = cache.get_package("react@18.2.0", &integrity).unwrap();
        assert_eq!(cached, retrieved);
    }
    ...
    fn test_has_package() {
        ...
        let pkg_file = temp.path().join("test.tgz");
        fs::write(&pkg_file, b"fake").unwrap();
        let integrity = cache.compute_integrity(&pkg_file).unwrap();
        cache.store_package("react@18.2.0", &pkg_file, &integrity).unwrap();
