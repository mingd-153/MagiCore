...
use mg_registry_server::auth::AuthService;
use mg_registry_server::model::Package;
use mg_registry_server::storage::RegistryStore;
...
async fn npm_routes_match() {
    ...
    let app = Router::new()
        .merge(mg_registry_server::npm::routes())
...
async fn pypi_upload_index_download() {
    ...
    let auth = std::sync::Arc::new(AuthService::new(Some("adm".to_string()), store.clone()));
    let app = Router::new()
        .merge(mg_registry_server::pypi::routes())
        .with_state((store, auth));
...
async fn users_persist_across_restart() {
    use mg_registry_server::auth::User;
...
async fn oci_routes_match() {
    ...
    let auth = std::sync::Arc::new(AuthService::new(Some("adm".to_string()), store.clone()));
    let app = Router::new()
        .merge(mg_registry_server::oci::routes())
        .with_state((store, auth));
...
fn scope_glob_matches() {
    use mg_registry_server::auth::scope_matches;
    assert!(scope_matches("*", "anything"));
    assert!(scope_matches("@megagate/*", "@megagate/core"));
    assert!(scope_matches("@megagate/*", "@megagate/core/extra"));
    assert!(!scope_matches("@megagate/*", "@other/pkg"));
