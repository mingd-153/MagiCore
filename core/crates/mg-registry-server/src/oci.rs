...
async fn v2_dispatch(
    ...
    match section {
        "blobs/uploads" => match tail {
            "/" | "" => blob_upload_start(&store, name, &query).await.map(|r| r.into_response()),
            uuid_tail => match method {
                Method::PATCH => blob_upload_chunk(&store, name, &uuid_tail[1..], &headers, body).await,
                Method::PUT => blob_upload_complete(&store, name, &uuid_tail[1..], &query, body).await,
...
async fn blob_get(
    ...
    match store.get_oci_blob(name, digest).await {
        Ok(Some(data)) => {
            ...
            let digest_header = format!("sha256:{}", digest.strip_prefix("sha256:").unwrap_or(digest));
            resp.headers_mut().insert(
...
async fn blob_upload_complete(
    ...
    let data: Vec<u8> = match store.oci_upload_path(name, uuid).await {
        Ok(Some(path)) => {
            let size = std::fs::metadata(&path).map_err(|_| StatusCode::NOT_FOUND)?.len() as usize;
            ...
            let mut read = 0;
            while read < size {
                let n = file.read(&mut buf[read..]).map_err(|_| StatusCode::INTERNAL_SERVER_ERROR)?;
                if n == 0 { break; }
                read += n;
            }
            buf.truncate(read);
