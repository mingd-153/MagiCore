...
use mgc_store::{Database, FileEntry, Layout, StoreIndex};
...
fn upsert_second_package_keeps_first() {
    let dir = test_layout();
    let mut index = open_index(&dir);
    let a = mgc_types::PackageId::parse("react@18.2.0").unwrap();
    let b = mgc_types::PackageId::parse("vue@3.4.0").unwrap();
    index
...
fn reopen_reads_same_index_from_msgpack() {
    let dir = test_layout();
    let pkg = mgc_types::PackageId::parse("lodash@4.17.21").unwrap();
    {
        let layout = Layout::new(dir.path().to_path_buf());
        let db = Database::open(&dir.path().join("store.db")).unwrap();
...
fn corrupt_msgpack_is_rebuilt_from_sqlite() {
    let dir = test_layout();
    let pkg = mgc_types::PackageId::parse("chalk@5.0.0").unwrap();
    {
        let layout = Layout::new(dir.path().to_path_buf());
        let db = Database::open(&dir.path().join("store.db")).unwrap();
        let mut index = StoreIndex::open(&layout, db).unwrap();
...
fn update_same_package_replaces_not_duplicates() {
    let dir = test_layout();
    let mut index = open_index(&dir);
    let pkg = mgc_types::PackageId::parse("preact@10.0.0").unwrap();
    index
...
fn prune_removes_only_unreferenced_old_blobs() {
    let dir = test_layout();
    let mut index = open_index(&dir);
    let pkg = mgc_types::PackageId::parse("pkg@1.0.0").unwrap();
    index
...
fn prune_referenced_blob_is_never_removed() {
    let dir = test_layout();
    let mut index = open_index(&dir);
    let pkg = mgc_types::PackageId::parse("keep@1.0.0").unwrap();
    index
...
fn digest_of_msgpack_is_stable_across_reopen() {
    let dir = test_layout();
    let pkg = mgc_types::PackageId::parse("stable@1.0.0").unwrap();
    let first = {
        let layout = Layout::new(dir.path().to_path_buf());
        let db = Database::open(&dir.path().join("store.db")).unwrap();
        let mut index = StoreIndex::open(&layout, db).unwrap();
