...
//! (bevy/esp32-rust/rust-lib: mgc add = cargo add + cargo fetch; write giữ [package.metadata.magicore])
...
pub fn parse_manifest(root: &Path, ecosystem: Ecosystem) -> MgResult<Manifest> {
    let content = std::fs::read_to_string(root.join("Cargo.toml"))
        .map_err(|e| mgc_types::MgError::Other(format!("read Cargo.toml: {e}")))?;
...
pub fn write_manifest(root: &Path, manifest: &Manifest) -> MgResult<()> {
    ...
    .map_err(|e| mgc_types::MgError::Other(format!("write Cargo.toml: {e}")))?;
    Ok(())
}
