# `mg-lockfile` — Unified Lockfile Engine

Crate cung cấp cơ chế quản lý lockfile thống nhất (`mg.lock`), chữ ký số BLAKE3, và công cụ chuyển đổi (import) các lockfile từ hệ sinh thái khác (npm, pnpm, yarn, bun).

---

## 🚀 Tính năng

1. **Serialize / Deserialize**: Chuyển đổi hai chiều định dạng TOML & JSON cho `mg.lock`.
2. **Checksum Integrity**: Tự động sinh và kiểm tra `mg.lock.sha256`.
3. **Chữ ký số BLAKE3 (Keyed Hash)**: Bảo vệ chống giả mạo lockfile khi có biến môi trường `MEGAGATE_LOCKFILE_KEY`.
...
```rust
use std::path::Path;
use mg_lockfile::read_lockfile_checked;
...
```rust
use std::path::Path;
use mg_lockfile::{Lockfile, write_lockfile};
...
```rust
use std::path::Path;
use mg_lockfile::import::import_legacy_lockfile_explicit;
use mg_types::Manifest;

let project_root = Path::new("./legacy-project");
let manifest = Manifest::new("app", mg_types::Ecosystem::Web);
