...
use mg_crypto::ed25519_signer::{Ed25519Signer, verify_signature};
...
fn bench_blake3_hash(c: &mut Criterion) {
    ...
    for size in [1024, 10_240, 102_400, 1_024_000].iter() {
        ...
        group.bench_with_input(BenchmarkId::from_parameter(size), size, |b, _| {
            b.iter(|| {
                Blake3Hasher::hash_bytes(black_box(&data))
            });
        });
    }
...
fn bench_ed25519_verify(c: &mut Criterion) {
    ...
    c.bench_function("ed25519_verify", |b| {
        b.iter(|| {
            verify_signature(black_box(&public_key), black_box(message), black_box(&signature)).unwrap()
        });
    });
}

criterion_group!(benches, bench_blake3_hash, bench_ed25519_sign, bench_ed25519_verify);
