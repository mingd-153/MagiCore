...
//!
//! Allows defining centralized dependency versions in `megagate.workspace.toml`
...
/// Load catalogs from `megagate.workspace.toml` if present.
pub fn load_workspace_catalogs(project_root: &Path) -> anyhow::Result<Option<WorkspaceCatalogs>> {
    let ws_path = project_root.join("megagate.workspace.toml");
    if !ws_path.exists() {
        return Ok(None);
    }
