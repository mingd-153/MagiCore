...
pub struct TlsConfig {
    /// Bypass cert validation (MEGAGATE_ALLOW_UNTRUSTED=1) - CHỈ dev/test
...
impl TlsConfig {
    ...
    pub fn from_env() -> Self {
        Self {
            allow_untrusted: std::env::var("MEGAGATE_ALLOW_UNTRUSTED")
                .map(|v| v == "1" || v.eq_ignore_ascii_case("true"))
                .unwrap_or(false),
            ca_bundle: std::env::var("MEGAGATE_CA_BUNDLE").ok(),
            client_cert: std::env::var("MEGAGATE_CLIENT_CERT").ok(),
            client_key: std::env::var("MEGAGATE_CLIENT_KEY").ok(),
            min_version: TlsVersion::V1_2,
        }
    }
