...
fn topo_chain_orders_levels() {
    let graph = WorkspaceGraph {
        ...
        edges: vec![
            mgc_workspace::WorkspaceEdge { from: 0, to: 1 },
            mgc_workspace::WorkspaceEdge { from: 1, to: 2 },
        ],
    };
    let levels = topo_levels(&graph).unwrap();
...
fn topo_fanout_same_level() {
    let graph = WorkspaceGraph {
        ...
        edges: vec![
            mgc_workspace::WorkspaceEdge { from: 0, to: 1 },
            mgc_workspace::WorkspaceEdge { from: 0, to: 2 },
        ],
    };
    let levels = topo_levels(&graph).unwrap();
...
fn topo_diamond() {
    let graph = WorkspaceGraph {
        ...
        edges: vec![
            mgc_workspace::WorkspaceEdge { from: 0, to: 1 },
            mgc_workspace::WorkspaceEdge { from: 0, to: 2 },
            mgc_workspace::WorkspaceEdge { from: 1, to: 3 },
            mgc_workspace::WorkspaceEdge { from: 2, to: 3 },
        ],
    };
    let levels = topo_levels(&graph).unwrap();
    assert_eq!(levels.len(), 3);
...
fn topo_cycle_detects() {
    let graph = WorkspaceGraph {
        ...
        edges: vec![
            mgc_workspace::WorkspaceEdge { from: 0, to: 1 },
            mgc_workspace::WorkspaceEdge { from: 1, to: 0 },
        ],
    };
...
fn filter_name_wildcard_patterns() {
    ...
    assert!(filter_matches("mgc-*", Path::new("any"), "mgc-workspace"));
...
"#,
    ...
    let rust_manifest = mgc_workspace::read_package_manifest(&rust_dir)
        .unwrap()
        .unwrap();
...
"#,
    ...
    let py_manifest = mgc_workspace::read_package_manifest(&py_dir)
        .unwrap()
        .unwrap();
...
fn test_computation_caching_roundtrip_and_invalidation() {
    ...
    let (should_rebuild, src_hash, comp_hash) =
        mgc_workspace::check_package_build_freshness(&pkg_root, &dep_hashes).unwrap();
    assert!(should_rebuild, "Fresh package must require build");

    // 2. Lưu cache sau khi build
    let saved =
        mgc_workspace::save_package_build_cache(&pkg_root, src_hash, dep_hashes.clone()).unwrap();
    ...
    let saved_modified = mgc_workspace::save_package_build_cache(
        &pkg_root,
        mgc_workspace::compute_package_source_hash(&pkg_root).unwrap(),
        dep_hashes.clone(),
    )
    .unwrap();
    assert_eq!(
    ...
    let (should_rebuild_dep_changed, _, _) =
        mgc_workspace::check_package_build_freshness(&pkg_root, &new_dep_hashes).unwrap();
    assert!(
