...
//! Tests for retry, ratelimit, and cache core logic
...
fn retry_fixed_returns_same_delay_for_any_attempt() {
    let strat = retry::RetryStrategy::Fixed(Duration::from_secs(5));
...
fn retry_exponential_doubles_each_attempt() {
    let strat = retry::RetryStrategy::Exponential {
...
fn ratelimit_default_values() {
    let config = ratelimit::RateLimitConfig::default();
