...
impl SearchCache {
    ...
    ///
    /// Cache location: ~/.megagate/search_cache.db
    /// Vị trí cache: ~/.megagate/search_cache.db
    ...
    fn cache_path() -> Result<PathBuf> {
        ...
        Ok(home.join(".megagate").join("search_cache.db"))
    }
    ...
    pub fn get(&self, query: &str) -> Result<Option<Vec<SearchResult>>> {
        ...
        let mut stmt = self.db.prepare(
            "SELECT results, timestamp, ttl FROM search_cache WHERE query = ?"
        )?;
