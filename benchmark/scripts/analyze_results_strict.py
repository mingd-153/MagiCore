...
REQUIRED_COLD = ['duration_seconds', 'disk_mb']
REQUIRED_WARM = ['duration_seconds']
...
def validate_result(data: Dict, filename: str) -> None:
    ...
    try:
        float(data['cold_install']['duration_seconds'])
        float(data['cold_install']['disk_mb'])
        float(data['warm_install']['duration_seconds'])
    except (ValueError, TypeError) as e:
        raise ValidationError(f"Non-numeric metric: {e}")
