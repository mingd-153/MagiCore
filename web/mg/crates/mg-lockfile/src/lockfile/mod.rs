...
//!
//! Dual format support:
//! - Binary (mg.lockb): bincode + custom header, fast load
//! - Text (mg.lock): TOML, human-readable, git-diffable

use serde::{Deserialize, Serialize};

pub mod verify;

use mg_core::{PackageId, Protocol, Resolution as CoreResolution};
use mg_resolver::Resolution as ResolverResolution;
...
impl Lockfile {
    ...
    pub fn compute_content_hash_v1(&self) -> String {
        let mut hasher = blake3::Hasher::new();
        hasher.update(b"mg-lockfile-v1");
...
impl LockfilePackage {
    ...
    pub fn from_resolver_resolution(res: &ResolverResolution, registry_url: &str) -> Self {
        let name_str = res.package_id.name().as_str();
        let filename = name_str.split('/').next_back().unwrap_or(name_str);
        Self {
