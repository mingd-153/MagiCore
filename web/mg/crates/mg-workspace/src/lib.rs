...
use mg_core::{LinkerMode, MgpmConfig, SecurityConfig, WorkspaceConfig};
...
impl Workspace {
    ...
    /// Checks for `mg.yaml`, `mg.yml`, `mg.toml`, or `package.json` with a workspaces field.
...
mod tests {
    ...
    fn test_discover_mg_yaml() {
        ...
        config.save(&root.join("mg.yaml")).unwrap();
