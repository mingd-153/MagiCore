//! MGPM Benchmarks
//!
//! Criterion-based benchmarks for core mg operations.
    ...
    for &size in [10, 100].iter() {
        group.bench_with_input(BenchmarkId::new("files", size), &size, |b, &n| {
            b.iter(|| {
                ...
                let store = mg_store::ContentStore::new(dir.path().join("store")).unwrap();
...
pub fn bench_lockfile_roundtrip(c: &mut Criterion) {
    ...
    group.bench_function("text", |b| {
        b.iter(|| {
            let _ = mg_lockfile::text::write_text(&lock, &text_path);
        });
    });
    group.bench_function("binary", |b| {
        b.iter(|| {
            let _ = mg_lockfile::binary::write_binary(&lock, &binary_path);
        });
    });
    group.finish();
    ...
    group.bench_function("text", |b| {
        b.iter(|| {
            let _ = mg_lockfile::text::read_text(&text_path);
        });
    });
    group.bench_function("binary", |b| {
        b.iter(|| {
            let _ = mg_lockfile::binary::read_binary(&binary_path);
        });
    });
    group.finish();
}
