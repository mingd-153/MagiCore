...
pub static FAVICON_ICO: &[u8] = include_bytes!("favicon.ico");
...
pub fn write_favicon(project_path: &Path) -> Result<(), ScaffoldError> {
    let public = project_path.join("public");
    std::fs::create_dir_all(&public).map_err(|e| ScaffoldError::IoError {
        context: "create public dir".to_string(),
        source: e,
    })?;
    let ico_path = public.join("favicon.ico");
    std::fs::write(&ico_path, FAVICON_ICO).map_err(|e| ScaffoldError::IoError {
        context: "write favicon.ico".to_string(),
        source: e,
    })?;
    Ok(())
}
