...
fn test_roundtrip_binary_text() {
    ...
    let text_path = dir.path().join("mg.lock");
    mg_lockfile::text::write_text(&original, &text_path).unwrap();
    let from_text = mg_lockfile::text::read_text(&text_path).unwrap();

    let binary_path = dir.path().join("mg.lockb");
    mg_lockfile::binary::write_binary(&original, &binary_path).unwrap();
    let from_binary = mg_lockfile::binary::read_binary(&binary_path).unwrap();
...
fn test_lockfile_validation() {
    ...
    assert_eq!(lock.version, mg_lockfile::LOCKFILE_VERSION);
...
fn test_migrate_v1_to_v2_roundtrip() {
    let mut lock = Lockfile::new(1, "npm");
    lock.version = mg_lockfile::lockfile::LOCKFILE_VERSION_V1;
...
fn test_migrate_v1_tampered_rejected() {
    ...
    assert!(matches!(
        result.unwrap_err(),
        mg_lockfile::LockfileError::ContentHashMismatch { .. }
    ));
}
...
fn test_text_auto_migrates_v1_to_v2() {
    ...
    let text_path = dir.path().join("mg.lock");
    mg_lockfile::text::write_text(&lock, &text_path).unwrap();

    let loaded = mg_lockfile::text::read_text(&text_path).unwrap();

    assert_eq!(loaded.version, mg_lockfile::LOCKFILE_VERSION);
...
fn test_binary_accepts_v1_and_v2() {
    ...
    let binary_path = dir.path().join("mg.lockb");
    mg_lockfile::binary::write_binary(&lock, &binary_path).unwrap();

    let loaded = mg_lockfile::binary::read_binary(&binary_path).unwrap();
    assert_eq!(loaded.version, mg_lockfile::lockfile::LOCKFILE_VERSION_V1);
    assert_eq!(loaded.packages.len(), 1);
}
...
fn test_compute_package_integrity_sri_format() {
    ...
    let integrity = mg_lockfile::pipeline::compute_package_integrity(data);
    assert!(
...
fn test_compute_package_integrity_deterministic() {
    ...
    let a = mg_lockfile::pipeline::compute_package_integrity(data);
    let b = mg_lockfile::pipeline::compute_package_integrity(data);
    assert_eq!(a, b);
}

#[test]
fn test_compute_package_integrity_different_inputs() {
    let a = mg_lockfile::pipeline::compute_package_integrity(b"package A content");
    let b = mg_lockfile::pipeline::compute_package_integrity(b"package B content");
    assert_ne!(a, b);
}
