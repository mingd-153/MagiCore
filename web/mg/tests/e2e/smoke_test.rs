...
use mg_resolver::solver::ResolvedDep;
...
impl mg_resolver::DependencyProvider for E2eProvider {
    fn get_versions(&self, package: &mg_core::PackageName) -> Vec<mg_core::Version> {
        self.packages
            .get(package.as_str())
            .map(|versions| {
                let mut v: Vec<mg_core::Version> = versions
                    .keys()
                    .filter_map(|s| mg_core::Version::parse(s).ok())
                    .collect();
                v.sort();
                v
            })
...
async fn e2e_smoke_resolve_and_lockfile() {
    ...
    assert!(
        root.join("mg.lock").exists(),
        "text lockfile should exist"
    );
    assert!(
        root.join("mg.lockb").exists(),
        "binary lockfile should exist"
    );

    let lock_text = fs::read_to_string(root.join("mg.lock")).expect("read lockfile");
    assert!(
...
fn e2e_smoke_install_from_lockfile() {
    ...
    let lf_path = root.join("mg.lock");
    mg_lockfile::text::write_text(&lockfile, &lf_path).expect("write lockfile");

    let loaded = mg_lockfile::text::read_text(&lf_path).expect("read lockfile");
...
fn e2e_smoke_lockfile_integrity() {
    ...
    let tmp = tempfile::tempdir().expect("tempdir");
    let lf_path = tmp.path().join("mg.lock");
    mg_lockfile::text::write_text(&lockfile, &lf_path).expect("write");

    let loaded = mg_lockfile::text::read_text(&lf_path).expect("read");
...
fn e2e_smoke_binary_lockfile_roundtrip() {
    ...
    let tmp = tempfile::tempdir().expect("tempdir");
    let bin_path = tmp.path().join("mg.lockb");
    mg_lockfile::binary::write_binary(&lockfile, &bin_path).expect("write binary");

    let loaded = mg_lockfile::binary::read_binary(&bin_path).expect("read binary");
    assert_eq!(loaded.packages.len(), 1);
