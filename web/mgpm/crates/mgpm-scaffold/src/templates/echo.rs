...
const FILES: &[(&str, &str)] = &[
    ("go.mod.hbs", include_str!("echo/go.mod.hbs")),
    (
        "cmd/server/main.go.hbs",
        include_str!("echo/cmd/server/main.go.hbs"),
    ),
    (
        "internal/router/router.go.hbs",
        include_str!("echo/internal/router/router.go.hbs"),
    ),
    (
        "internal/handler/health.go.hbs",
        include_str!("echo/internal/handler/health.go.hbs"),
    ),
    (
        "internal/handler/item.go.hbs",
        include_str!("echo/internal/handler/item.go.hbs"),
    ),
    (
        "internal/model/item.go.hbs",
        include_str!("echo/internal/model/item.go.hbs"),
    ),
    (
        "internal/repository/item.go.hbs",
        include_str!("echo/internal/repository/item.go.hbs"),
    ),
    ("Dockerfile.hbs", include_str!("echo/Dockerfile.hbs")),
    ("go.sum.hbs", include_str!("echo/go.sum.hbs")),
    (".gitignore.hbs", include_str!("echo/.gitignore.hbs")),
    (
        ".env.example.hbs",
        include_str!("echo/.env.example.hbs"),
    ),
    ("README.md.hbs", include_str!("echo/README.md.hbs")),
];

use crate::engine::{ProjectCreated, ScaffoldContext, StaticScaffolder};

pub struct EchoScaffolder(StaticScaffolder);

impl ScaffoldEngine for EchoScaffolder {
    fn name(&self) -> &str {
        "echo"
    }
...
fn create_temp_dir() -> Result<PathBuf, ScaffoldError> {
    let base = std::env::temp_dir().join("mgpm-echo");
...
fn build_engine() -> Box<dyn ScaffoldEngine> {
    ...
    Box::new(EchoScaffolder(StaticScaffolder::new(path)))
}
...
mod test {
    ...
    #[test]
    fn test_echo_engine_creates_project() {
        ...
        vars.insert("name".to_string(), "my-echo-api".to_string());

        let ctx =
            ScaffoldContext::new("my-echo-api", temp.path().join("output")).with_vars(vars);
