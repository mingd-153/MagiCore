...
const FILES: &[(&str, &str)] = &[
    ("Cargo.toml.hbs", include_str!("actix/Cargo.toml.hbs")),
    (".env.example.hbs", include_str!("actix/.env.example.hbs")),
    ("src/main.rs.hbs", include_str!("actix/src/main.rs.hbs")),
    (
        "src/models/mod.rs.hbs",
        include_str!("actix/src/models/mod.rs.hbs"),
    ),
    (
        "src/models/item.rs.hbs",
        include_str!("actix/src/models/item.rs.hbs"),
    ),
    (
        "src/handlers/mod.rs.hbs",
        include_str!("actix/src/handlers/mod.rs.hbs"),
    ),
    (
        "src/handlers/items.rs.hbs",
        include_str!("actix/src/handlers/items.rs.hbs"),
    ),
    (
        "migrations/20240101000001_create_items.sql.hbs",
        include_str!("actix/migrations/20240101000001_create_items.sql.hbs"),
    ),
    ("Dockerfile.hbs", include_str!("actix/Dockerfile.hbs")),
    (".gitignore.hbs", include_str!("actix/.gitignore.hbs")),
    ("README.md.hbs", include_str!("actix/README.md.hbs")),
];

use crate::engine::{ProjectCreated, ScaffoldContext, StaticScaffolder};

pub struct ActixScaffolder(StaticScaffolder);

impl ScaffoldEngine for ActixScaffolder {
    fn name(&self) -> &str {
        "actix"
    }
...
fn create_temp_dir() -> Result<PathBuf, ScaffoldError> {
    let base = std::env::temp_dir().join("mgpm-actix");
...
fn build_engine() -> Box<dyn ScaffoldEngine> {
    ...
    Box::new(ActixScaffolder(StaticScaffolder::new(path)))
}
...
mod test {
    ...
    #[test]
    fn test_actix_engine_creates_project() {
        ...
        vars.insert("name".to_string(), "my-actix-api".to_string());
        vars.insert("version".to_string(), "1.0.0".to_string());

        let ctx =
            ScaffoldContext::new("my-actix-api", temp.path().join("output")).with_vars(vars);
    ...
    #[test]
    fn test_actix_build_engine() {
        let engine = build_engine();
        assert_eq!(engine.name(), "actix");
    }
}
