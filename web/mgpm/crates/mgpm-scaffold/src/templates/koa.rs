...
const FILES: &[(&str, &str)] = &[
    (
        "package.json.hbs",
        include_str!("koa/package.json.hbs"),
    ),
    (
        "tsconfig.json.hbs",
        include_str!("koa/tsconfig.json.hbs"),
    ),
    (
        "src/index.ts.hbs",
        include_str!("koa/src/index.ts.hbs"),
    ),
    ("src/app.ts.hbs", include_str!("koa/src/app.ts.hbs")),
    (
        "src/routes/items.ts.hbs",
        include_str!("koa/src/routes/items.ts.hbs"),
    ),
    (
        "src/middleware/error.ts.hbs",
        include_str!("koa/src/middleware/error.ts.hbs"),
    ),
    (
        "src/types/index.ts.hbs",
        include_str!("koa/src/types/index.ts.hbs"),
    ),
    ("Dockerfile.hbs", include_str!("koa/Dockerfile.hbs")),
    (".gitignore.hbs", include_str!("koa/.gitignore.hbs")),
    (
        ".env.example.hbs",
        include_str!("koa/.env.example.hbs"),
    ),
    ("README.md.hbs", include_str!("koa/README.md.hbs")),
];

use crate::engine::{ProjectCreated, ScaffoldContext, StaticScaffolder};

pub struct KoaScaffolder(StaticScaffolder);

impl ScaffoldEngine for KoaScaffolder {
    fn name(&self) -> &str {
        "koa"
    }
...
fn create_temp_dir() -> Result<PathBuf, ScaffoldError> {
    let base = std::env::temp_dir().join("mgpm-koa");
...
fn build_engine() -> Box<dyn ScaffoldEngine> {
    ...
    Box::new(KoaScaffolder(StaticScaffolder::new(path)))
}
...
mod test {
    ...
    #[test]
    fn test_koa_engine_creates_project() {
        ...
        vars.insert("name".to_string(), "my-koa-api".to_string());
        vars.insert("version".to_string(), "1.0.0".to_string());

        let ctx =
            ScaffoldContext::new("my-koa-api", temp.path().join("output")).with_vars(vars);
    ...
    #[test]
    fn test_koa_build_engine() {
        let engine = build_engine();
        assert_eq!(engine.name(), "koa");
    }
}
