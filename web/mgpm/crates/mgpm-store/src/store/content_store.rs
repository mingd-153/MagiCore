...
impl ContentStore {
    ...
    fn import_with_method(&self, src: &Path, dst: &Path) -> io::Result<ImportMethod> {
        ...
        if let Some(method) = self.try_hardlink(src, dst) {
            return Ok(method);
        }
    ...
    fn try_hardlink(&self, src: &Path, dst: &Path) -> Option<ImportMethod> {
        match fs::hard_link(src, dst) {
            Ok(_) => {
                let src_meta = fs::metadata(src).ok();
                let entry = FileEntry {
                    hash: self.hash_file(dst).ok()?,
