...
fn test_dep_symlink_depth_pnpm_style() {
    ...
    let mgpm_root = dir.path().join(".mgpm");
    assert!(mgpm_root.join("virtual_store").exists(), "virtual_store dir");
    ...
    print_tree(&mgpm_root, 0);

    // Dep symlink is now at virtual_store/<pkg_dir>/node_modules/<dep>
    let dep_link = mgpm_root.join("virtual_store").join("root-pkg_1.0.0_e3b0c442").join("node_modules").join("dep-pkg");
